...
export async function GET(
  ...
  try {
    ...
    console.log('🔍 Coach', coachData.coach_name, 'accessing athlete', athleteId)
    ...
    const avgRPE = totalSessions > 0 
      ? Math.round((recentPerformance.reduce((sum, log) => sum + (log.rpe || 0), 0) / totalSessions) * 10) / 10
      : 0

    const avgQuality = totalSessions > 0
      ? Math.round((recentPerformance.reduce((sum, log) => sum + (log.completion_quality || 0), 0) / totalSessions) * 10) / 10
      : 0
    ...
    console.error('❌ Unexpected error in coach/athlete detail:', error)
    return NextResponse.json(
      { 
        success: false, 
