...
interface BrandMenuBrowserProps {
  brand: Brand
  onBack: () => void
  onFoodLogged?: () => void
}

export default function BrandMenuBrowser({
  brand,
  onBack,
  onFoodLogged,
}: BrandMenuBrowserProps) {
  ...
  const searchMenu = async (query: string) => {
    ...
    try {
      ...
      const brandName = brand.fatsecret_brand_filter || brand.brand_name
      ...
      <View style={styles.section}>
        ...
        <View style={styles.searchBox}>
          ...
          <TextInput
            ...
            placeholder={`Search ${brand.brand_name} products...`}
            value={searchQuery}
            onChangeText={(text) => {
              setSearchQuery(text)
              searchMenu(text)
