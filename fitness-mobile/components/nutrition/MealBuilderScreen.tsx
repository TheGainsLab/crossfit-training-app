...
type ScreenView = 'builder' | 'portion' | 'search' | 'searchDetails' | 'savedFoods'

/**
 * MealBuilderScreen - Full-screen meal builder flow
 *
 * This is a swap-out screen component (not a modal) that replaces the main view
 * when active. Uses internal view switching instead of nested modals to prevent
 * iOS modal stacking issues.
 *
 * Views:
 * - builder: Main meal builder interface
 * - portion: Portion adjustment for selected ingredient
 * - search: Food search interface
 * - searchDetails: Food selection/details after search
 * - savedFoods: Saved foods picker (ingredients/restaurants/brands)
 */
export default function MealBuilderScreen({
  ...
  onLogMeal,
  preselectedMealType,
}: MealBuilderScreenProps) {
  const [currentView, setCurrentView] = useState<ScreenView>('builder')
  ...
  const handleBack = () => {
    ...
    switch (currentView) {
      ...
      case 'savedFoods':
        ...
        break
      default:
        // On builder view, check for unsaved items
  ...
  const handleSearchFoodSelected = (food: { food_id: string; food_name: string }) => {
    setSearchSelectedFood({ foodId: food.food_id, foodName: food.food_name })
    setCurrentView('searchDetails')
  }

  const handleSearchFoodAdded = (foodData: any) => {
    // Add food from search flow
    const newItem: MealItem = {
      id: Date.now().toString() + Math.random(),
      food_id: foodData.food_id,
      food_name: foodData.food_name,
      amount: foodData.number_of_units,
      unit: 'serving',
      calories: foodData.calories,
      protein: foodData.protein,
      carbs: foodData.carbohydrate,
      fat: foodData.fat,
    ...
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <ScrollView style={styles.content}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.categoryTabs}>
            {getSortedCategories().map((cat) => (
              ...
              >
                ...
                <Text
                  style={[styles.categoryTabText, selectedCategory === cat && styles.categoryTabTextActive]}
                >
        ...
        <View style={styles.addButtonsContainer}>
          <TouchableOpacity style={styles.addButton} onPress={() => setCurrentView('search')}>
          ...
          <View style={styles.savedFoodsRow}>
            ...
            >
              <Text style={styles.savedFoodButtonText}>Brands</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <Card style={styles.section}>
          ...
          {mealItems.length === 0 ? (
            <Text style={styles.emptyText}>No items yet. Search and add foods to build your meal.</Text>
          ) : (
        ...
        {mealItems.length > 0 && (
          <Card style={styles.totalsCard}>
            ...
            <Text style={styles.totalsMacros}>
              ...
              {Math.round(totals.carbs * 10) / 10}g Carbs • {Math.round(totals.fat * 10) / 10}g Fat
            </Text>
          </Card>
        )}
      </ScrollView>
        ...
        {saving ? (
          ...
          <>
            <View style={styles.footerRow}>
              ...
              {onLogMeal && (
                <TouchableOpacity
                  style={[
                    styles.footerButton,
                    styles.logAndSaveButton,
                    (mealItems.length === 0 || !mealName.trim()) && styles.footerButtonDisabled,
                  ]}
                  onPress={async () => {
            ...
            <View style={styles.footerRow}>
              ...
              <TouchableOpacity style={[styles.footerButton, styles.cancelButton]} onPress={handleBack}>
...
// Saved Foods Picker View Component (not a modal)
interface SavedFoodsPickerViewProps {
  mode: 'ingredients' | 'restaurants' | 'brands'
  onBack: () => void
  onFoodSelected: (food: any) => void
}

function SavedFoodsPickerView({ mode, onBack, onFoodSelected }: SavedFoodsPickerViewProps) {
  ...
  const getModeTitle = () => {
    ...
    if (mode === 'brands') return 'Brand Items'
    return 'Select Food'
  }
