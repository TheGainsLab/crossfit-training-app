-- Default food sources for Meal Builder, Restaurants, and Brands
-- These are admin-editable defaults shown to users for quick selection
...
CREATE TABLE IF NOT EXISTS default_ingredients (
  ...
  name TEXT NOT NULL,
  emoji TEXT,
  search_term TEXT NOT NULL,           -- What to search in FatSecret API
  category TEXT NOT NULL,              -- protein, carb, vegetable, fat, fruit, dairy, other
  sort_order INT DEFAULT 0,
  is_active BOOLEAN DEFAULT true,
  usage_count INT DEFAULT 0,           -- Track popularity for sorting
...
-- DEFAULT RESTAURANTS
-- ============================================
CREATE TABLE IF NOT EXISTS default_restaurants (
  id SERIAL PRIMARY KEY,
  name TEXT NOT NULL,                  -- Display name: "Chipotle"
  emoji TEXT,
  fatsecret_name TEXT NOT NULL,        -- Exact FatSecret name: "Chipotle Mexican Grill"
  aliases TEXT[],                      -- Common variations for search
  sort_order INT DEFAULT 0,
  is_active BOOLEAN DEFAULT true,
...
-- DEFAULT BRANDS
-- ============================================
CREATE TABLE IF NOT EXISTS default_brands (
  id SERIAL PRIMARY KEY,
  name TEXT NOT NULL,
  emoji TEXT,
...
CREATE POLICY "Anyone can read default_ingredients" ON default_ingredients
  FOR SELECT USING (true);

CREATE POLICY "Anyone can read default_restaurants" ON default_restaurants
  FOR SELECT USING (true);

CREATE POLICY "Anyone can read default_brands" ON default_brands
  FOR SELECT USING (true);

-- Only service role can modify (admin via Supabase dashboard or API)
